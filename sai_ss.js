...
return;
if(i==1)
obj.parentNode.style.backgroundColor=this.ibut_hc? this.ibut_hc : 'white';
else if(i==2)
...
slideHTML+='<a href="'+this.imgs[picidx][2]+'"'+specs+'">'
}
...
if(this.go('trace'+this.issid+'_0')){
var spans=document.getElementsByTagName('span'), idt=new RegExp('trace'+this.issid+'_');
for (var i_tem = 0; i_tem < spans.length; i_tem++)
if(idt.test(spans[i_tem].id))
spans[i_tem].style.backgroundImage = 'url(cssimages/sl_nobg.png)';
this.go('trace'+this.issid+'_'+this.keeptrack()).style.backgroundImage = 'url(cssimages/select_sl.png)';
}
if (this.jumpto)
