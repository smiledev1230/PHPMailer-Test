...
class db
{
            ...
            if ($image) {
                ...
                if (($extension != "jpg") && ($extension != "jpeg") && ($extension != "png") && ($extension != "gif")) {
                    ...
                    if (!$copied) {
                        ...
                        if ($thumb) {
                            $sql = "INSERT INTO images (name) VALUES ('$image_name')";

                            mysqli_query($this->conn, $sql) or die (mysql_error());
                            $id = mysql_insert_id();

                            if ($id > 0) {
                                $sql1 = "UPDATE `images` SET `order_id` = '$id' WHERE `id`= $id ";
                                mysqli_query($this->conn, $sql) or die (mysql_error());
                            }
                        }
    ...
    public function removeimgforcat($image_id, $cat_id)
    {
        ...
        if ($image_id) {
            $sql = "UPDATE `images` SET `cat_id` = '$new_listcat' WHERE `id`= $image_id";
            mysqli_query($this->conn, $sql) or die (mysql_error());
        }
    }
    ...
    {
        if ($cat_id) {
            $sql = "UPDATE `category` SET `order_id` = '$order_id' WHERE `id`= $cat_id";
            mysqli_query($this->conn, $sql) or die (mysql_error());
        }
    }
